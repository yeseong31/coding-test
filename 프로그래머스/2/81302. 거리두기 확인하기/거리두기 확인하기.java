...
public class Solution {
    ...
    private boolean isDistanced(char[][] place, Point p) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (nx < 0 || nx >= 5 || ny < 0 || ny >= 5 || place[nx][ny] != 'P') continue;
    ...
    private boolean isPossiblePlace(char[][] place) {
        ...
        for (int x = 0; x < 5; x++) {
            for (int y = 0; y < 5; y++) {
                if (place[x][y] == 'P') persons.add(new Point(x, y));
            }
        }

        for (Point person : persons) {
            if (!isDistanced(place, person)) return false;
        }
    ...
    public int[] solution(String[][] places) {
        ...
        for (int i = 0; i < places.length; i++) {
            ...
            answer[i] = isPossiblePlace(place) ? 1 : 0;
        }
