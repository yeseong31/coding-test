...
# ---------------------------------------------------------------------------
# zip() 함수는 자신이 감싼 iterator 중 어느 하나가 끝날 때까지 튜플을 반환하는데
# 긴 iterator의 뒷부분을 버리지 않도록 하려면 itertools의 zip_longest를 사용하면 된다.
from itertools import zip_longest
names = ['Cecilia', '길동', '둘리', 'Rosalina']
cnts = [len(n) for n in names]

names.append('zelda')
for name, cnt in zip_longest(names, cnts):
    print(f'{name}: {cnt}')
